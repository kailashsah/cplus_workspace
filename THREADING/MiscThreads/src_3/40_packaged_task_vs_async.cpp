...
/*
	...
	1.	(a) std::packaged_task allows us to get the std::future "bounded" to some callable,
		(b) and then control when and where this callable will be executed without the need of that future object.
...
public:
	...
	std::future<T> enqueue(task_type task) {
...

		{
			...
			tasks_.push(std::move(task)); // we can store packaged_task<> in vectors/queue,but not async (1) 
		}
		cv_.notify_one();
		return res;
	}
...
/*
	Such functionality cannot be implemented with std::async. We need to return an std::future from enqueue().If we called std::async there(even with deferred policy) and return std::future, then we would have no option how to execute the callable in worker(). -- (IMP)

	Note that you cannot create multiple futures for the same shared state(given, futures are non - copyable).
*/
