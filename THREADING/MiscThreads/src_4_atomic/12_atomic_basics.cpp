...
void run_ref_inside_atomic_int() {
	{
		...
		int val{ 5 };
		int& ref = val;                     // (2)
		std::atomic<int> atomicRef(ref);	// same as atomicRef(val);
		++atomicRef;                        // (1)

		std::cout << "ref: " << ref << std::endl;							// 5
		std::cout << "atomicRef.load(): " << atomicRef.load() << std::endl; // 6
